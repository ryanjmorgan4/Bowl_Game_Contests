...
Bowls2018 <- read.csv(file="C:\\Users\\Ryan\\Desktop\\Home\\BowlGames2018\\2018Bowl.csv",header=TRUE)

key2018 <- Bowls2018$Numeric.Key

key2018[is.na(key2018)] <- 0

picks2018 <- Bowls2018[,c(14:21)]

bowlGameScenarios(picks2018,key2018)

combination_percent_chance_of_winning(picks=picks2018, key=key2018, probs=fpi_probs$Team2_Win_Prob, num_sims=2000, number_of_runs = 20 )
...
pick_agreement <- function(){
  ...
  Pick_Team_1<- Bowls2018$Text_Ryan == Bowls2018[,5:12]
...
Pick_Team_1 <- as.data.frame(Pick_Team_1)
ret <- apply(Pick_Team_1, MARGIN = 1, FUN=sum)
...
Bowls2018 %>% filter(ret == 8)
One_person_pick <- Bowls2018 %>% filter(ret == 7)
