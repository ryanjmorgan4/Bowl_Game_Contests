...
score <- function(key, picks){
  ...
  semisChamp = sum(key[c(36,37,41)]== picks[c(36,37,41)])
...
(Oklahoma v Alabama, Clemson v Notre Dame, national championship)
...
TEST1<- rbind.data.frame(all_poss, semis_1[1],semis_1[2],semis_1[3])

TEST2<- rbind.data.frame(all_poss, semis_2[1],semis_2[2],semis_2[3])

TEST3<- rbind.data.frame(all_poss, semis_3[1],semis_3[2],semis_3[3])

TEST4<- rbind.data.frame(all_poss, semis_4[1],semis_4[2],semis_4[3])

TEST5<- rbind.data.frame(all_poss, semis_5[1],semis_5[2],semis_5[3])

TEST6<- rbind.data.frame(all_poss, semis_6[1],semis_6[2],semis_6[3])

TEST7<- rbind.data.frame(all_poss, semis_7[1],semis_7[2],semis_7[3])

TEST8<- rbind.data.frame(all_poss, semis_8[1],semis_8[2],semis_8[3])

all_poss <- cbind.data.frame(TEST1,TEST2,TEST3,TEST4,TEST5,TEST6,TEST7,TEST8)
return(all_poss)
...
simulate_key <- function(picks, key, probs){
  ...
  completed_games <- key[1:numColmpleted(key)]
...
fpi_probs <- read.csv(file="C:\\Users\\Ryan\\Desktop\\Home\\BowlGames2018\\fpi_probs.csv",header=TRUE)


simulate_key(picks= picks2018, key=key2018, probs = fpi_probs$Team2_Win_Prob)
...
```{r}
mult_simulations <- function(picks=picks2018, key=key2018, probs=fpi_probs$Team2_Win_Prob, num_sims=10){
...
Favorites and upsets:
```{r}
all_favorites <- round(fpi_probs$Team2_Win_Prob)+1
...
all_upsets <- all_favorites + (-1)^(1+all_favorites)
all_upsets[41] = 3


winning(key=all_favorites, picks = picks2018)
compare(key = all_favorites, picks=picks2018)

winning(key=all_upsets, picks = picks2018)
compare(key=all_upsets, picks =picks2018)
  
```
Early Odds:

You need two games completed in order to find simulations. So, find the percent chances for the four scenarios of the first two games, then multiply and add.

```{r}
scen_1_key <- c(1,1,rep(0,39))
scen_2_key <- c(1,2,rep(0,39))
scen_3_key <- c(2,1,rep(0,39))
scen_4_key <- c(2,2,rep(0,39))
...
scen_1_sims <- combination_percent_chance_of_winning(picks = picks2018, key = scen_1_key, probs = fpi_probs$Team2_Win_Prob, num_sims = 1000, number_of_runs = 20)

scen_2_sims <- combination_percent_chance_of_winning(picks = picks2018, key = scen_2_key, probs = fpi_probs$Team2_Win_Prob, num_sims = 1000, number_of_runs = 20)

scen_3_sims <- combination_percent_chance_of_winning(picks = picks2018, key = scen_3_key, probs = fpi_probs$Team2_Win_Prob, num_sims =1000, number_of_runs = 20)

scen_4_sims <- combination_percent_chance_of_winning(picks = picks2018, key = scen_4_key, probs = fpi_probs$Team2_Win_Prob, num_sims = 1000, number_of_runs = 20)


scen_1_prob*scen_1_sims+scen_2_prob*scen_2_sims+scen_3_prob*scen_3_sims+scen_4_prob*scen_4_sims
