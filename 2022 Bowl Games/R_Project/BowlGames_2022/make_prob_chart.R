...
probabilities_plus_cat <- (as.data.frame(read.csv("C:\\Users\\Ryan\\OneDrive\\Desktop\\Home\\2022 Bowl Games\\Prob_Tracker_Cat.csv", stringsAsFactors = FALSE), stringsAsFactors = FALSE))
probabilities <- (as.data.frame(read.csv("C:\\Users\\Ryan\\OneDrive\\Desktop\\Home\\2022 Bowl Games\\Prob_Tracker.csv", stringsAsFactors = FALSE), stringsAsFactors = FALSE))

probabilities_plus_cat %>% gather(value = Probs, key = Person,Aidan:Tie)%>%
  filter(!is.na(Probs)) %>%
  ggplot(aes(x=Games_Played, y = Probs, group = Person, color = Person, lty=Person))+geom_line(size=1)+theme(legend.position = "bottom")+
...
Scores_plus_cat <- (as.data.frame(read.csv("C:\\Users\\Ryan\\OneDrive\\Desktop\\Home\\2022 Bowl Games\\Score_Tracker_Cat.csv", stringsAsFactors = FALSE), stringsAsFactors = FALSE))
Scores <-(as.data.frame(read.csv("C:\\Users\\Ryan\\OneDrive\\Desktop\\Home\\2022 Bowl Games\\Score_Tracker.csv", stringsAsFactors = FALSE), stringsAsFactors = FALSE))


Scores_plus_cat %>% gather(value = Score, key = Person,Aidan:Wrigley)%>%
  filter(!is.na(Score)) %>%
  ggplot(aes(x=Games_Played, y = Score, group = Person, color = Person, lty=Person))+geom_line(size=1)+theme(legend.position = "bottom")+
