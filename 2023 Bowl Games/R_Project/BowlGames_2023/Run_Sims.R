...
convert_picks_to_1_2 <- function(picks_vector, team1_team2_dataframe, 
                                 team1_champ = "Michigan",
                                 team2_champ = "Washington",
                                 team3_champ = "Texas",
                                 team4_champ = "Alabama"){
...
calculate_score <- function(key_vector, picks_vector, 
                            index_ny6_games = c(33, 34, 36, 40, 41, 42, 43), 
                            index_semis = c(41, 42, 43),
                            index_champ = c(43),
...
#Rose Bowl: 
# 1: Michigan    2: Alabama

#Sugar Bowl:
# 1: Washington 2: Texas
...
BowlGames_2023_pluscat <- as.data.frame(read.csv("/Users/Ryan/Desktop/Home/2023 Bowl Games/bowlgames_2023.csv", stringsAsFactors = FALSE), stringsAsFactors = FALSE)

BowlGames_2023 <- BowlGames_2023_pluscat %>% select(-Wrigley_Picks)


og_2023 <- run_simulations_function(picks_dataset = BowlGames_2023, numberofsimulations = 10000)
og_2023$time_to_run
og_2023$Probability_Of_Winning %>% arrange(-Percent_Won)
og_2023$Probability_Of_Winning

cat_2023 <- run_simulations_function(picks_dataset = BowlGames_2023_pluscat, numberofsimulations = 10000)
cat_2023$time_to_run
cat_2023$Probability_Of_Winning %>% arrange(-Percent_Won)
cat_2023$Probability_Of_Winning
